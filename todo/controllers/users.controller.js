...
const createUser = async (req, res) => {
  ...
  if (
    !isValid(user.id) ||
    (isValid(user.id) && !isValidString(user.id))
  ) {
    response.success = false;
    response.code = 400;
  ...
  if (
    !isValid(user.email) ||
    (isValid(user.email) && !isValidString(user.email))
  ) {
    response.success = false;
    response.code = 400;
...
//         success: false,
//         code: 404,
//         message: "Invalid request, user item does not exist",
//         data: null,
//         error: null,
//         resource: req.originalUrl,
//       });
//     }
//     if (user.userId.toString() !== res.locals.userId) {
